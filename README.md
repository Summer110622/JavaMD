...
コンパイルはページ内で再利用する Java 環境で行い、ECJ の標準ライブラリ索引を保持します。コードを編集した後も、索引の初期化を毎回やり直しません。直前にコンパイルに成功したコードと完全に一致する場合は、生成済みのクラスとコンパイル診断を再利用します。

プログラムの実行結果はキャッシュしません。Run を押すたびに新しい Java 実行環境で `main()` を呼び出し、static 変数、例外、スレッド、`System.exit()` の動作を保ちます。実行側には ECJ を読み込まず、小さな実行用 JAR のみを渡します。ステータスバーには Run 全体の所要時間も表示します。初回には引き続き Java 17 と ECJ の読み込み・初期化が必要です。
